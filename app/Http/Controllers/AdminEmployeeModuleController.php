<?php namespace App\Http\Controllers;
	...
	class AdminEmployeeModuleController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Customer Type","name"=>"customer_type", "join"=>"channels,channel_description"];
			$this->col[] = ["label"=>"Channel","name"=>"channel_id", "channel_id"=>false ,"join"=>"channels,channel_code", "visible"=>false];
			...
			$this->col[] = ["label"=>"Payment Terms","name"=>"payment_terms_id","join"=>"payment_terms,payment_terms_description" , "visible"=>false];
			...
			//$this->form[] = ["label"=>"Email Address","name"=>"email_address","type"=>"email","required"=>TRUE,"validation"=>"required|min:1|max:255|email|unique:employee","placeholder"=>"Please enter a valid email address"];
	        ...
	        */
			...
			if(CRUDBooster::getCurrentMethod() == 'getIndex' ){
				...
				"label"=>"Export",
				"icon"=>"fa fa-download","url"=>CRUDBooster::mainpath('GetExtractEmployee').'?'.urldecode(http_build_query(@$_GET))];
			}
	    ...
	    */
	    public function hook_query_index(&$query) {
			...
			$query->where(function($sub_query){
				...
				$sub_query->where('employee.approval_status_id', 	$create_item_status);
				$sub_query->orWhere('employee.approval_status_id',	$update_item_status);
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
	        //Your code here
			$data_status =	 	Statuses::where('status_description', 'ACTIVE')->value('id');
			$channel = 			Channel::where('channel_description', 'EMPLOYEE')->value('id');
			...
			$postdata["employee_code"]				= $emp_code; //description
			$postdata["customer_type"]				= $channel; //description
			$postdata["channel_id"]					= $channel; //channel code
	    ...
	    */
	    public function hook_after_add($id) {        
	        //Your code here
			$item_data = Employee::where('id',$id)->get()->toArray();	
			EmployeeApproval::insert($item_data);
			...
			foreach ($approvers as $approvers_list){
				...
				if($for_approval->encoder_privilege_id == $approver_privilege_for->id){
					...
					foreach ($send_to as $send_now){
							...
							$config['content'] =  CRUDBooster::myName(). " has created Employee with Customer/Location Name ".$for_approval->customer_location_name." at Employee Module!";
						

						$config['to'] = CRUDBooster::adminPath('employee_approval/edit/'.$for_approval->id);
			...
			CRUDBooster::redirect(CRUDBooster::mainpath(),"Employee with Employee Code ".$employee_data->employee_code." has been created successfully.","info");
			//CRUDBooster::redirect(CRUDBooster::mainpath(),"Employee has been created and pending for approval.","info");
	    }
	    ...
	    */
	    public function hook_before_edit(&$postdata,$id){        
			...
			/*
			EmployeeApproval::where('id', $id)->update([
				...
				'customer_location_name'	=>  $postdata["customer_location_name"],
				...
				'bank_address'	=>  $postdata["bank_beneficiary"],
	    ...
	    */
	    public function hook_after_edit($id) {
			//Your code here 
			//$employee_infos = Employee::where('id',$id)->first();
			
			/*EmployeeApproval::where('id',$employee_infos->id)->update([
				...
				'action_type'	=>  $employee_infos->action_type,
				'updated_by'	=>  $employee_infos->updated_by,
				'updated_at'	=>  $employee_infos->updated_at,
				'approval_status_id'	=>  $employee_infos->approval_status_id
			]);*/
			...
			foreach ($approvers as $approvers_list){
				...
				if($for_approval->encoder_privilege_id == $approver_privilege_for->id){
					...
					foreach ($send_to as $send_now){
							...
							$config['content'] =  CRUDBooster::myName(). " has edited Employee with Customer/Location Name ".$for_approval->customer_location_name." at Employee Module!";
			...
			*/
			...
			CRUDBooster::redirect(CRUDBooster::mainpath(""),"The Employee with Employee Code ".$employee_data->employee_code." has been edited successfully.","info");
	    }
		...
		*/
		...
		public function GetExtractEmployee() {

            $filename = 'Employee - ' . date("d M Y - h.i.sa");
			$sheetname = 'Employee'.date("d-M-Y");
			...
			Excel::create($filename, function ($excel) {
				$excel->sheet('orders', function ($sheet) {	
					...
					$orderData = DB::table('employee')
					->leftjoin('channels', 'employee.channel_id','=', 'channels.id')
					//->leftjoin('customer', 'customer.customer','=', 'customer.id')
					->leftjoin('countries', 'employee.country_id','=', 'countries.id')
					->leftjoin('cities', 'employee.city_id','=', 'cities.id')		
					->leftjoin('states', 'employee.state_id','=', 'states.id')				
					->leftjoin('countries as country', 'employee.country_id','=', 'country.id')																	
					//->leftjoin('payment_terms', 'employee.payment_terms_id','=', 'payment_terms.id')
					//->leftjoin('bea_pricelist', 'employee.bea_pricelist_id','=', 'bea_pricelist.id')
					->leftjoin('statuses', 'employee.status_id','=', 'statuses.id')	
					->leftjoin('cms_users as created', 'employee.created_by','=', 'created.id')
					->leftjoin('cms_users as updated', 'employee.updated_by','=', 'updated.id')
					->select( 	'employee.*', 
								'channels.*', 
					...
					$ordeDataLines = $orderData->orderBy('employee.id','asc')->get();
					$blank_field = '';
					$store_inv = '';
					$counter=0;
					...
					foreach ($ordeDataLines as $orderRow) {
						...
						$orderItems[] = array(
							...
							$orderRow->employee_code, 		
							$orderRow->customer_location_name, 	
                    ...
                    $sheet->getStyle('A1:V1')->applyFromArray(array(
                        'fill' => array(
