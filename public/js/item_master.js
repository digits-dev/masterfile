...
$(document).ready(function() {
    ...
    $('#ttp').on('change keyup click', function() {
        ...
        if(add_action){
            if(lc != ''){
                var temp_ttpp = parseFloat(ttp_p/temp_lc)-1;
				$('#ttp_percentage').val(parseFloat(temp_ttpp * 100).toFixed(2));
            }
        }
        else if(edit_action){
            if(lc != ''){
    ...
    $('#ttp_percentage').on('change keyup click', function() {
        
        var ttp_ppercentage = this.value;
        var tax_code = $('#tax_codes_id').val();
        var lc = parseFloat(document.getElementById('landed_cost').value);
        var temp_lc = lc;
        ...
        if(add_action){
            if(lc != ''){
                var tempmulttp = (parseFloat(ttp_ppercentage)/100) + 1;
    	        $('#ttp').val(parseFloat(temp_lc*tempmulttp).toFixed(2));
            }
        }
        else if(edit_action){
            if(lc != ''){
